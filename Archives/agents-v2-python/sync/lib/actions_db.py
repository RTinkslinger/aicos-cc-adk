...
def create_action(
    ...
    # TODO: Add request_id column to actions_queue table
    # Idempotency check: if request_id is provided, return existing row instead of inserting
    if request_id:
        conn = _get_conn()
        try:
            with conn.cursor(cursor_factory=psycopg2.extras.RealDictCursor) as cur:
                cur.execute("SELECT 1 FROM actions_queue WHERE request_id = %s", (request_id,))
                existing = cur.fetchone()
                if existing:
                    cur.execute("SELECT * FROM actions_queue WHERE request_id = %s", (request_id,))
...
def upsert_from_notion(
    ...
    """
    # TODO: Add request_id column to actions_queue table
    # Idempotency check: if request_id is provided, return existing row instead of upserting
    if request_id:
        conn = _get_conn()
        try:
            with conn.cursor(cursor_factory=psycopg2.extras.RealDictCursor) as cur:
