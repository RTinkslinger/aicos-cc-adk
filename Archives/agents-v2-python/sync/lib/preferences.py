...
def log_outcome(
    ...
    Returns:
        The inserted row ID.
    """
    # TODO: Add request_id column to action_outcomes table
    # Idempotency check: if request_id is provided, return existing row instead of inserting
    if request_id:
        conn = _get_conn()
        try:
            cur = conn.cursor()
            cur.execute("SELECT 1 FROM action_outcomes WHERE request_id = %s", (request_id,))
            existing = cur.fetchone()
            if existing:
                cur.execute("SELECT id FROM action_outcomes WHERE request_id = %s", (request_id,))
