...
def create_thread(
    ...
    # TODO: Add request_id column to thesis_threads table
    # Idempotency check: if request_id is provided, return existing row instead of inserting
    if request_id:
        conn = _get_conn()
        try:
            with conn.cursor(cursor_factory=psycopg2.extras.RealDictCursor) as cur:
                cur.execute("SELECT 1 FROM thesis_threads WHERE request_id = %s", (request_id,))
                existing = cur.fetchone()
                if existing:
                    cur.execute("SELECT * FROM thesis_threads WHERE request_id = %s", (request_id,))
...
def update_thread(
    ...
    # TODO: Add request_id column to thesis_threads table
    # Idempotency check: if request_id is provided, return existing row instead of updating
    if request_id:
        conn = _get_conn()
        try:
            with conn.cursor(cursor_factory=psycopg2.extras.RealDictCursor) as cur:
...
def enqueue_sync(table_name: str, record_id: int, operation: str, payload: dict, request_id: str | None = None) -> int:
    """Add a failed Notion write to the retry queue."""
    # TODO: Add request_id column to sync_queue table
    # Idempotency check: if request_id is provided, return existing row instead of inserting
    if request_id:
        conn = _get_conn()
        try:
            with conn.cursor() as cur:
                cur.execute("SELECT 1 FROM sync_queue WHERE request_id = %s", (request_id,))
                existing = cur.fetchone()
                if existing:
                    cur.execute("SELECT id FROM sync_queue WHERE request_id = %s", (request_id,))
